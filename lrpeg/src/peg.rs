...
impl PEG {
    ...
    fn rule_definition(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        let res = {
            ...
            self.builtin_xid_identifier(pos, input, None, None)
            .and_then(|node| {
                let pos = node.end;
                list.push(node);
                self.rule_ws(pos, input)
            ...
            .and_then(|node| {
                ...
                self.match_terminal(pos, input, Terminal::Literal_0)
                    .map(|end| Node::new(Rule::Terminal, pos, end, None, None))
                    .ok_or(pos).or_else(|_| Ok(Node::new(Rule::Terminal, pos, pos, None, None)))
            })
            .and_then(|node| {
                let pos = node.end;
                list.push(node);
    ...
    fn rule_expression(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        let res = {
            ...
            self.rule_sequence_marker(pos, input)
            .and_then(|node| {
                let pos = node.end;
                list.push(node);
                self.rule_ws(pos, input)
    ...
    #[allow(non_snake_case)]
    fn rule_sequence_marker(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        let key = (pos, Rule::sequence_marker);

        if let Some(res) = self.rule_memo.get(&key) {
            return res.clone();
        }

        let res = self.match_terminal(pos, input, Terminal::Literal_1)
                    .map(|end| Node::new(Rule::sequence_marker, pos, end, None, None))
                    .ok_or(pos)
            .or_else(|_| {
            let mut list = Vec::new();
            let start = pos;
    ...
    fn rule_alternative(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        loop {
            ...
            .and_then(|node| {
                let pos = node.end;
                list.push(node);
                self.rule_alternative(pos, input)
            })
            .and_then(|node| {
                let pos = node.end;
                list.push(node);
    ...
    fn rule_primary(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        let res = {
            ...
            self.match_terminal(pos, input, Terminal::Literal_11)
                    .map(|end| Node::new(Rule::Terminal, pos, end, None, None))
                    .ok_or(pos)
            .and_then(|node| {
                let pos = node.end;
            ...
            self.match_terminal(pos, input, Terminal::Literal_12)
                    .map(|end| Node::new(Rule::Terminal, pos, end, None, None))
                    .ok_or(pos)
            .and_then(|node| {
                let pos = node.end;
            ...
            .and_then(|node| {
                let pos = node.end;
                list.push(node);
                match self.match_terminal(pos, input, Terminal::Literal_2)
                    .map(|end| Node::new(Rule::Terminal, pos, end, None, None))
                    .ok_or(pos) {
                    Ok(_) => Err(pos),
                    Err(_) => Ok(Node::new(Rule::MustNotMatch, pos, pos, None, None)),
            ...
            self.match_terminal(pos, input, Terminal::Literal_13)
                    .map(|end| Node::new(Rule::Terminal, pos, end, None, None))
                    .ok_or(pos)
            .and_then(|node| {
                let pos = node.end;
    ...
    fn rule_string_literal(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        let res = self.match_terminal(pos, input, Terminal::Regex)
                    .map(|end| Node::new(Rule::string_literal, pos, end, None, None))
                    .ok_or(pos)
    ...
    fn rule_regex(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        let res = self.match_terminal(pos, input, Terminal::re)
                    .map(|end| Node::new(Rule::regex, pos, end, None, None))
                    .ok_or(pos);
    ...
    fn rule_ws(&mut self, pos: usize, input: &str) -> Result<Node, usize> {
        ...
        let res = {
            ...
            .or_else(|_| self.builtin_whitespace(pos, input, None, None)
                        .map(|node| { Node {
                            rule: Rule::ws,
                            start: node.start,
                            end: node.end,
